...
The corruption harness owns image transformations. For a GPU run, each manifest observation must reference the resulting RGB file through `materialized_image_path`; when that field is absent, the runner uses `image_path`. Paths are resolved under `--dataset-root`. Never point different corruption variants at the clean source merely to complete a run: materialize the declared condition first.
...
```shell
python rgb_baseline_cli.py --manifest ./artifacts/track5-production/track5-manifest.json --dataset-root ./datasets/sid-set/observations --output-dir ./artifacts/rgb-baseline --device cuda --batch-size 16
```

The profile records wall-clock seconds, throughput, batch size, peak allocated GPU bytes, device, precision, failure count, and retry count. Pass `--retry-once` only when a failed batch should be retried once and recorded.
