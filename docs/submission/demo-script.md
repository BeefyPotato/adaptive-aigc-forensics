...
| 0:35–1:00 (25s) | Run `python submission_inference_cli.py --image-dir <directory> --bundle-dir artifacts/issue-7-fusion-v2 --rgb-checkpoint <community-forensics-384.safetensors> --signal-model <signal-model.json> --output predictions.json --device auto --batch-size 8`, then show `predictions.json`. Say: “Both expert artifacts are verified before scoring. Each sorted record has only `image_path` and a finite fusion probability `pred`; it is not a provenance verdict.” Do not substitute `rgb_cli.py`. |
