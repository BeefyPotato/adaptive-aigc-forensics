...
| Bundle SHA-256 | Not applicable to direct directory inference; any experiment bundle must supply its own SHA-256 before a result is published |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Current candidate uses the frozen 384-pixel RGB expert | Candidate: raw RGB-only; not a fusion claim | `config/community-forensics-models.json` → `models.384`; `rgb_cli.py` | `6fbfb1d` base; replace on release | N/A for direct CLI | Checkpoint SHA-256 above | README, Devpost | Documented; human review still required |
