...
class SubmissionInferenceTests(unittest.TestCase):
    ...
    def test_artifact_gate_rejects_each_model_and_rgb_runtime_binding(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            cases = [
                ...
                ("RGB", "rgb_normalizer", "mean"),
                ("RGB", "rgb_normalizer", "scale"),
                ("RGB", "rgb_normalizer", "center_crop"),
                ("RGB", "rgb_normalizer", "resize_interpolation"),
                ("RGB", "rgb_normalizer", "channel_order"),
                ("RGB", "rgb_normalizer", "input_range"),
                ("RGB", "rgb_normalizer", "tensor_dtype"),
                ("RGB", "rgb_normalizer", "tensor_layout"),
            ]
    ...
    def test_artifact_gate_binds_actual_runtime_normalization_and_shared_revision(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            bundle_dir, rgb_path, signal_path, bundle, receipt, rgb_sha, signal_sha = (
                self._deployment_fixture(root)
            ...
            for name, runtime_patch in (
                (
                    "mean",
                    patch.object(
                        subject,
                        "IMAGENET_MEAN",
                        np.asarray([0.1, 0.2, 0.3], dtype=np.float32)[:, None, None],
                    ),
                ),
                (
                    "scale",
                ...
                (
                    ...
                    patch.object(
                        subject,
                        "SHARED_OBSERVATION_PREPROCESSING_VERSION",
                        "shared-preprocessing-v2",
                    ),
                ),
                (
                    "shared geometry",
                    patch.object(subject, "SHARED_EXPERT_GEOMETRY", {384: 441}),
                ),
            ):
                with self.subTest(binding=name):
                    with (
                        ...
                        base_patches[6], base_patches[7], runtime_patch,
                        self.assertRaisesRegex(ValueError, "RGB"),
                    ):
                        subject.validate_submission_artifacts(
                            bundle_dir,
                            rgb_checkpoint=rgb_path,
    ...
    def test_public_entry_point_validates_before_constructing_models(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            with (
                ...
                with self.assertRaisesRegex(ValueError, "bad artifacts"):
                    subject.run_submission_inference(
                        root,
                        frozen_generation_directory=root,
                        rgb_checkpoint=root / "rgb",
                        signal_model=root / "signal",
                        output_path=output,
                        device="cpu",
                    )
    ...
    def test_public_cpu_entry_point_never_probes_cuda_and_owns_backends(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            output = root / "predictions.json"
            bundle = {"bundle_revision": "test"}
            with (
                patch.object(subject, "validate_submission_artifacts", return_value=bundle),
                patch.object(subject, "_cuda_is_available", side_effect=AssertionError("CUDA probed")),
                patch.object(subject, "CommunityForensicsBackend") as rgb_type,
                patch.object(subject, "SignalModelBackend") as signal_type,
                patch.object(
                    subject,
                    "_run_submission_with_backends",
                    return_value=[{"image_path": "a.jpg", "pred": 0.5}],
                ) as runner,
            ):
                rows = subject.run_submission_inference(
                    root,
                    frozen_generation_directory=root,
                    rgb_checkpoint=root / "rgb",
                    signal_model=root / "signal",
            ...
            self.assertEqual(rgb_type.call_args.kwargs["device"], "cpu")
            signal_type.assert_called_once_with(root / "signal", bundle)
            runner.assert_called_once()

    def test_public_unavailable_cuda_fails_before_artifacts_and_models(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            with (
    ...
    def test_non_finite_expert_output_preserves_existing_publication(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                subject._run_submission_with_backends(
                    root,
                    root,
                    output,
    ...
    def test_directory_output_is_sorted_relative_and_byte_stable(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(subject, "load_frozen_bundle", return_value=bundle), patch.object(
                ...
                subject._run_submission_with_backends(root, root, output, ConstantBackend([0.0, 1.0]), ConstantBackend([0.0, -1.0]))
    ...
    def test_directory_inference_uses_production_signal_pixel_scale(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(subject, "load_frozen_bundle", return_value=bundle):
                rows = subject._run_submission_with_backends(
                    root,
                    root,
                    output,
                    ConstantBackend([0.0]),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(subject, "load_frozen_bundle", return_value=bundle), patch.object(
                ...
                with self.assertRaisesRegex(ValueError, "one logit"):
                    subject._run_submission_with_backends(root, root, output, ConstantBackend([]), ConstantBackend([0.0]))
    ...
    def test_empty_input_writes_valid_empty_array(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(subject, "load_frozen_bundle", return_value=bundle):
                subject._run_submission_with_backends(root, root, output, ConstantBackend([]), ConstantBackend([]))
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(subject, "load_frozen_bundle", return_value=bundle):
                with self.assertRaisesRegex(ValueError, "unreadable or corrupt"):
                    subject._run_submission_with_backends(root, root, output, ConstantBackend([0.0]), ConstantBackend([0.0]))
            self.assertFalse(output.exists())
